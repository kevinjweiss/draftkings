def dk_optimizer_ownership_studlock(year, week, player_target_file):
    ...
    rb_df_studs = rb_df_small.drop(rb_df_small[rb_df_small.DK_Points < 22].index)
    rb_df_studs = rb_df_studs['Player']
    wr_df_studs = wr_df_small.drop(wr_df_small[wr_df_small.DK_Points < 22].index)
    wr_df_studs = wr_df_studs['Player']

    if (len(rb_df_studs)>0):
        all_lineups = pd.concat([all_lineups.drop(all_lineups[~all_lineups.RB1.isin(rb_df_studs)].index),
    ...
    if(len(wr_df_studs)>0):
        all_lineups = pd.concat([all_lineups.drop(all_lineups[~all_lineups.WR1.isin(wr_df_studs)].index),
